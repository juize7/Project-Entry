...
export default function AddFloorContent() {
  ...
  const handleSubmit = () => {
    console.log("Form submitted:", { buildingName, floorName, floorArea, mediaFiles })
    router.back()
  }
  ...
  return (
    <div className="flex flex-col h-screen w-full max-w-md mx-auto bg-[#F7F7F7]">
      <header className="bg-white px-4 py-3 flex items-center shadow-sm">
        ...
        <h1 className="flex-1 text-center text-[17px] font-normal">添加楼层</h1>
        <div className="w-5" />
      </header>
          ...
          <section className="bg-white rounded-lg p-4 space-y-6">
            <div className="space-y-4">
              <div className="space-y-2">
                <h2 className="text-[17px] font-normal">楼层名称</h2>
                <Input
                  value={floorName}
                  onChange={(e) => setFloorName(e.target.value)}
                  className="bg-white border-gray-200"
                  placeholder="输入楼层名称，例如：3层"
                />
              </div>
              <div className="space-y-2">
                <h2 className="text-[17px] font-normal">楼层面积（平方米）</h2>
                <Input
                  type="number"
                  value={floorArea}
