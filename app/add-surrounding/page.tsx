...
export default function AddSurrounding() {
  const router = useRouter()
  const [surroundingName, setSurroundingName] = useState("")
  ...
  const handleSubmit = () => {
    // 处理表单提交
    console.log("Form submitted:", { surroundingName, mediaFiles })
  }
  ...
  return (
    <div className="flex flex-col h-screen w-full max-w-md mx-auto bg-[#F7F7F7]">
      <header className="bg-white px-4 py-3 flex items-center shadow-sm">
        ...
        <h1 className="flex-1 text-center text-[17px] font-normal">添加外围</h1>
        <div className="w-5" /> {/* Spacer for centering */}
      </header>

      <ScrollArea className="flex-grow">
        <div className="p-4 space-y-6">
          {/* Surrounding Name and Media Upload Combined */}
          <section className="bg-white rounded-lg p-4 space-y-6">
            <div className="space-y-4">
              <div className="space-y-2">
                <h2 className="text-[17px] font-normal">外围名称</h2>
                <Input
                  value={surroundingName}
                  onChange={(e) => setSurroundingName(e.target.value)}
