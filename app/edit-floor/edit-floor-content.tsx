...
export default function EditFloorContent() {
  const router = useRouter()
  const searchParams = useSearchParams()
  const buildingName = searchParams.get("building") || "未知楼栋"
  const floorName = searchParams.get("floor") || "未知楼层"
  ...
  const handleSubmit = () => {
    console.log("Submitting:", { buildingName, name, area, mediaFiles })
    router.back()
  }
        ...
        <div className="p-4 space-y-6">
          <section className="bg-white rounded-lg p-4 space-y-4">
            <div>
              ...
              <Input id="buildingName" value={buildingName} readOnly className="mt-1 bg-gray-100" />
            </div>
            <div>
              <label htmlFor="floorName" className="block text-sm font-medium text-gray-700">
                楼层名称
              </label>
              <Input
                id="floorName"
